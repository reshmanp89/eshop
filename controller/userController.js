...
exports.addToCart = async (req, res) => {
  try {
   ...
   const updatedProduct= await Product.findByIdAndUpdate(productId, { $inc: { quantity: -(quantity || 1) } },{new:true});
...
exports.removeFromCart= async(req,res)=>{
  ...
  //  console.log('userId',userId);
   const cart= await Cart.findOne({user:userId})
    ...
    if(productIndex === -1)
    {
      return res.status(404).render('cart',{message:"Product not found in the cart"})
    }
    ...
    await Product.findByIdAndUpdate(productId,{$inc:{quantity:removedQuantity}},{new:true})
